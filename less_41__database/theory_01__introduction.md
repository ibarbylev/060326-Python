...
*Допускает дополнительную установку нативной C-библиотеку клиента MySQL*  
*(опционально, для повышения производительности),*  
*но прекрасно работает и на "чистом" Python (`pip install mysql-connector-python`)*
...
* Иногда возможны проблемы с установкой некоторых версий на Windows:
  * Рекомендация: "откатить" на более старую версию пакета, 
  * например, если имеются проблемы с версией `9.5.0`, то есть смысл попробовать `9.3.0`:
    ```bash
...
* Требует нативную C-библиотеку клиента MySQL (поэтому сложнее в установке)
