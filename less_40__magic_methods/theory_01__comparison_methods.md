...
### Пример того, как хорошо жить с `NotImplemented` (а без него — плохо)
...
class Person:
    ...
    def __gt__(self, other):
        # Здесь Person ещё на умеет сравнивать себя с Employee
        if not isinstance(other, Person):
            return NotImplemented
        return self.age > other.age
...
class Employee:
    ...
    def __lt__(self, other):
        # "Учим" Employee сравнивать себя с Person
        # if isinstance(other, (Employee, Person)):
        #     return self.age < other.age
        return NotImplemented
...
print(p > e)   # p.__gt__(e) -→  NotImplemented -→ e.__lt__(p) -→ True
```